...
- 
- ⚠️ 编译加速：
- 1.全局开启编译加速，首次编译生成缓存，二次编译自动提速；
- 2.免费缓存共 10G，建议单次同时编译固件不超过 2 个；
- 3.缓存数量、剩余空间会影响加速效果，可自行研究调试；
- 4.编译速度整体提升 10% 以上，效果受型号、插件影响；
- 5.缓存引发固件问题时，前往 Action - Caches 清空缓存，重新完整编译。
